...
class MasterChef:
    ...
    def evaluate_flop(self):
        ...
        available_ranks = tile(range(2,14+1),4)
        available_suits = tile(range(1,4+1),13)

        # remove the cards already in my hand or on table
        for i in range(52,1-1,-1):
        ...
        for i in range(0,len(available_ranks)):
            for j in range(0,i):
    ...
    def evaluate_hand(self,card_ranks,card_suits):
        ...
        if sorted_rank_frequency[0] == 3:
            ...
            if sorted_rank_frequency[1] > 1:
                high_pair = max(array(range(2,14+1))[nonzero(rank_frequency>1)[0]*(nonzero(rank_frequency>1)[0]!=high_trip-2)])
        ...
        # look for trip
        if sorted_rank_frequency[0] == 3:
            kickers = flipud(sort(range(2,14+1)*(rank_frequency==1)))
        ...
        if sorted_rank_frequency[0] == 2:
            ...
            if sorted_rank_frequency[1] == 2:
                ...
                kicker = max(array(range(2,14+1))[nonzero(rank_frequency>0)[0]*(nonzero(rank_frequency>0)[0]!=high_pair-2)*(nonzero(rank_frequency>0)[0]!=next_pair-2)])
